...
Version 0.8 — 2d8 Exploding Dice, Stat/Damage Flip, Future Features
...
Four stats, designed to be setting-agnostic and immediately recognizable. Values range 0–3, keeping 2d8 modifiers meaningful without breaking the curve. Every class cares about at least two stats.

| Stat | Governs | Combat Role |
|------|---------|-------------|
| Strength (STR) | Melee attack accuracy, physical narrative checks, brute force interactions | Added to **melee attack rolls** (hit chance). Damage comes from the weapon, not the stat. |
| Dexterity (DEX) | Ranged attack accuracy, initiative/turn order, dodge, stealth, finesse checks | Added to **ranged attack rolls** (hit chance). Damage comes from the weapon, not the stat. Determines who acts first. |
| Intelligence (INT) | Magic attack accuracy, skill card effectiveness, narrative card checks, trap detection, information gathering | Added to **magic attack rolls** (hit chance for spell weapons with `"magic": true` flag). Damage comes from the weapon. In sci-fi settings reads as technical brilliance; in fantasy reads as arcane knowledge. |
...
**Key design rule: Stats determine accuracy (how often you hit). Weapons determine damage (how hard you hit).** This prevents modifier stacking from breaking the 2d8 curve and makes weapon upgrades from packs feel meaningful.

| Attack Type | Hit Roll | Damage Bonus | Example |
|-------------|----------|--------------|---------|
| Melee (physical weapon) | 2d8 + STR + zone tags | + weapon bonus | Iron Sword (+1): roll 2d8+STR to hit, damage tiers + 1 |
| Ranged (physical weapon) | 2d8 + DEX + zone tags | + weapon bonus | Short Bow (+1): roll 2d8+DEX to hit, damage tiers + 1 |
| Ranged Magic (spell weapon) | 2d8 + INT + zone tags | + weapon bonus | Fire Bolt (+2): roll 2d8+INT to hit, damage tiers + 2 |
...
| Class | Weapon Slot | Hit Stat | Rationale |
|-------|-------------|----------|-----------|
| Warrior | Melee weapon only | STR | Frontline fighter, must close distance to deal damage |
| Ranger | Ranged weapon only | DEX | Keeps distance, attacks from connected zones |
| Mage | Ranged spell weapon only | INT | Magic weapons carry the `"magic": true` flag |
| Rogue | Melee OR ranged (player's choice) | STR (melee) or DEX (ranged) | Flexibility IS the rogue's class identity |
...
Weapon cards carry a `"magic": true` flag for spell weapons. This tells the system to use INT for the attack roll instead of STR or DEX. All weapons carry a damage bonus (+0 to +3) that adds to the damage tier result. Weapon damage bonus is the primary source of damage scaling — stats only affect hit chance.
...
3. **Resolution:** If combat: enemy figures appear on zones, combat plays out using the 2d8 system with zone movement and engagement rules. If choice: player picks A or B. If check: player rolls d20 against a stat. If event: immediate effect applies.
...
| Equipment | Weapons, armor, tools, consumable items. Weapons carry a damage bonus (+0 to +3) that adds to damage tiers — this is the primary source of damage scaling. Weapons may also carry a `"magic": true` flag indicating they use INT for attack rolls. Any class can use any equipment (weapon slot is class-restricted). | Magic sword (+2), plasma rifle (+1), healing potion, shield, sniper scope, enchanted cloak, Fire Bolt staff (+2, magic) |
...
## 10.3 Dice System (2d8 Tier-Based with Exploding Crits)

Core attack: roll 2d8 + stat modifier (STR/DEX/INT based on weapon type). The roll determines whether and how well you hit. Damage is determined by the result tier + weapon bonus.

| Roll Total | Result | Damage |
|------------|--------|--------|
| 2–6 | Miss | 0 |
| 7–10 | Graze | 1 + weapon bonus |
| 11–13 | Hit | 2 + weapon bonus |
| 14–15 | Strong Hit | 3 + weapon bonus |
| 16+ | Critical | 4 + weapon bonus |

**Stats add to the attack roll** (accuracy). **Weapons add to the damage tier** (how hard you hit). These two systems never stack into the same number, preventing the modifier-stacking problem that would break a 2d6 curve.

**Why 2d8 over 2d6:** The 2d8 range (2–16, 15 values) provides ~36% more breathing room than 2d6 (2–12, 11 values). A +3 stat modifier is meaningful but doesn't dominate the curve. Zone tags can add small situational bonuses (+1) without pushing into constant crits. The bell curve centered on 9 keeps results predictable enough for tactical planning while leaving room for exciting variance.

### Exploding Crits

**On a natural 16 (both d8s showing 8), the dice explode.** Roll an additional d8 and add it to the damage. If that d8 also rolls an 8, it explodes again. This chains indefinitely.

- Natural 16 chance: 1/64 (~1.6%)
- Exploding once: adds d8 to crit damage (average +4.5)
- Double explosion (8 then 8 again): 1/512 chance (~0.2%) — legendary moment

Exploding crits are rare enough to be genuinely exciting but don't affect balance in any meaningful way. They create the "I can't believe that just happened" stories that players share. The visual of dice exploding on screen with escalating effects reinforces the tabletop fantasy.

### Stat Modifier Impact on 2d8

| Stat Bonus | Miss Chance | Graze | Hit | Strong Hit | Crit |
|------------|-------------|-------|-----|------------|------|
| +0 | 33% | 38% | 19% | 8% | 2% |
| +1 | 25% | 38% | 22% | 11% | 4% |
| +2 | 17% | 36% | 25% | 14% | 8% |
...
Even at +3, miss chance stays meaningful (~11%) and crits are achievable but not dominant (~12%). The curve degrades gracefully with modifiers.
...
| d8 | Shadowed zone crit bonus damage, exploding crit chain |
| d10/d12 | Legendary weapon/ability effects (planned) |
| d20 | Skill checks, narrative card resolutions, interactions |
...
Tavern Brawl encounter with 4 graph-based zones (Balcony, Main Floor, Bar, Back Room). 2 enemies with distinct AI behaviors (Bandit Brute — aggressive, Bandit Archer — ranged). Hero with passive ability, equipment loadout, target selection. 2d8 tier-based combat with exploding crits on natural 16, color-coded combat log and dice tray showing bonus dice. Stats determine hit chance, weapons determine damage. Engagement and retreat mechanics. Zone tags (elevated, cover, shadowed). Validated: combat loop is satisfying, zones create interesting positioning decisions, tier system is fast and readable.
...
Tweak numbers: health pools (8 + CON × 2), weapon damage bonuses, damage tier thresholds, cards per run, gold per tier, pack prices, figure passive balance, cooldown durations, exploding crit feel. This is where the fun is found through iteration.
...
The following features are designed and documented but intentionally deferred to later development phases. They solve problems that may not exist yet and should only be implemented once the core systems have been tested enough to confirm the need.

## 16.1 Narrative Die

A dedicated die rolled during narrative card reveals, separate from the combat dice system. Purpose: add variety to narrative moments so the same card plays differently across runs. Potential implementations include a themed d12 with narrative outcomes (Discovery, Danger, Betrayal, etc.), an escalating tension die that grows from d4 to d12 as the run progresses, or a simple fortune die that modifies narrative card outcomes. This pairs naturally with the runtime AI narrative generation stretch goal — the LLM could interpret the card + die result combination to generate unique text each time.

**When to implement:** After 15+ full playthroughs of the scene-based run system. If narrative moments feel repetitive despite hidden card information, this feature solves that problem. If they don't, it's unnecessary complexity.

## 16.2 Enemy Tactics/AI Modifier Cards

Optional cards that slot onto enemy figures to modify or override their baked-in AI behavior. A Bandit Brute is naturally aggressive, but attaching a "Cunning" tactics card makes it retreat at low health. A "Berserk" card makes it deal double damage but take extra. A "Pack Leader" card gives nearby allies bonus attacks.

**Benefits:** Multiplies encounter variety without creating new figures (5 enemies × 4 tactics cards = 20 distinct encounters). Fits the player-as-DM fantasy — you're programming enemy behavior. Adds another collectible card type to the pack economy.

**When to implement:** After the core enemy roster feels stale from repeated playthroughs. If 5 starter enemies with baked-in behaviors still feel fresh after 20 runs, defer further. If they feel predictable after 10 runs, this becomes a priority.
